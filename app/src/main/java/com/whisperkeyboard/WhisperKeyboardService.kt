...
class WhisperKeyboardService : InputMethodService() {
    ...
    private fun preloadModel(from: String) {
        Thread {
            try {
                ...
                if (!MoonshineEngine.isLoaded(m)) {
                    handler.post { updateStatus("Loading $m model...") }
                    MoonshineEngine.applyThreadPref(this)
                    val ok = MoonshineEngine.ensureModel(this, m, getLang())
    ...
    /**
     * @param firstSession true for the first normal -> Moonshine auto-start:
     * no chunk cuts; record at least 30s, auto-stop on 2s silence, hard cap 60s.
     */
    private fun startRecording(firstSession: Boolean = false) {
        ...
        if (!ModelManager.isModelReady(this, selModel) && !MoonshineEngine.isLoaded(selModel)) {
        ...
        // model not in memory -> start recording NOW, load concurrently
        if (!MoonshineEngine.isLoaded(selModel)) preloadModel("startRecording")
    ...
    override fun onStartInputView(info: android.view.inputmethod.EditorInfo?, restarting: Boolean) {
        ...
        // normal-keyboard -> Moonshine-keyboard switch.
    ...
    override fun onDestroy() {
        ...
        outstandingListener?.let {
            OutstandingStore.unregister(it)
        }
