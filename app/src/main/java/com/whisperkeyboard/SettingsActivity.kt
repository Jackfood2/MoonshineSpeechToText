    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        spinnerThreads.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: android.view.View?, pos: Int, id: Long) {
                ...
                saved()
                val n = MoonshineEngine.applyThreadPref(this@SettingsActivity)
                Toast.makeText(this@SettingsActivity, "Moonshine will use $n threads (auto)", Toast.LENGTH_SHORT).show()
            }
        ...
        spinnerModel.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: android.view.View?, pos: Int, id: Long) {
                ...
                if (m != prev) {
                    ...
                    Thread {
                        if (ModelManager.isModelReady(this@SettingsActivity, m)) {
                            val ok = MoonshineEngine.ensureModel(this@SettingsActivity, m, prefs.getString("lang","en") ?: "en")
        ...
        findViewById<Button>(R.id.btnTestModel).setOnClickListener {
            ...
            if (!ModelManager.isModelReady(this, m) && !MoonshineEngine.isLoaded(m)) {
            ...
            Thread {
                ...
                val msg = if (ok) "TEST OK: $m loaded in ${ms}ms" else "TEST FAILED: $m (${MoonshineEngine.lastError})"
        ...
        findViewById<Button>(R.id.btnClearModels).setOnClickListener {
            ...
            // also clear internal moonshine cache markers
            try {
                val cacheRoot = ai.moonshine.voice.ModelCache.defaultRoot(this)
                cacheRoot.listFiles()?.forEach { it.deleteRecursively() }
            } catch (_: Throwable) {}
            if (files.isNullOrEmpty()) {
                Toast.makeText(this, "No marker models to clear, cache cleared", Toast.LENGTH_SHORT).show()
                refreshModelInfo()
            } else {
                var freed = 0L
                for (f in files) { freed += f.length(); f.delete() }
                MoonshineEngine.unloadIfIdle()
                Toast.makeText(this, "Cleared ${files.size} marker(s), cache cleared", Toast.LENGTH_LONG).show()
                refreshModelInfo()
        ...
        lifecycleScope.launch {
            try {
                withContext(Dispatchers.IO) {
                    if (!MoonshineEngine.isLoaded(lastModel) && ModelManager.isModelReady(applicationContext, lastModel)) {
